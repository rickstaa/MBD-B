...
text(x_point,mean(q(:,1))+0.3*pi,(strcat('var =',num2str(var(q(:,1))))));
xlabel('t [s]');
ylabel('\alpha ]rad]');
title(title_str);
subplot(3,1,2)
...
%% ODE function handle
function [qdd] = ODE_func(t,q)
q_now = num2cell(q',1);
qdd   = subs_qdd(q_now{1:end});
qdd   = [q(4);q(5);q(6);qdd];
end
...
matlabFunction(simplify(qdp),'vars',[alpha,beta,gamma,alpha_d,beta_d,gamma_d],'File','subs_qdd');                                 % Create function handle of EOM in terms of generalised coordinates
