...
% Both bars vertical up and zero speed
x0           = [0.5*pi 0.5*pi 0 0];
x_dp_1       = state_calc(x0,parms);

% Both bars horizontal to the right and zero speed
x0           = [0 0 0 0];
x_dp_2       = state_calc(x0,parms);

% Both bars horizontal and with an initial angular speed on both bars of 60
% rpm
x0           = [0 0 2*pi 2*pi];
x_dp_3       = state_calc(x0,parms);

% Calculate velocities
x1_p         = -(parms.L1/2)*sin(x0(1))*x0(3);
y1_p         =  (parms.L1/2)*cos(x0(1))*x0(3);
x2_p         = x1_p - (parms.L1/2)*sin(x0(1))*x0(3) - (parms.L2/2)*sin(x0(2))*x0(4);
y2_p         = y1_p + (parms.L1/2)*cos(x0(1))*x0(3) + (parms.L2/2)*cos(x0(2))*x0(4);
...
x0           = [0.5*pi 0.5*pi 0 0];
x_dp_e       = state_calc(x0,parms);

% f - Both bars vertical up and with an initial angular speed of omega = 60
% rpm on bar 1
x0          = [0.5*pi 0.5*pi -2*pi 0];
x_dp_f      = state_calc(x0,parms);

% f - Both bars vertical up and with an initial angular speed of omega = 60
% rpm on bar 1
parms.Fg    = [parms.m1*parms.g;0;-10*(parms.L1/2);parms.m2*parms.g;0;0];        % Add a 10 N force in the y direction in B
x0          = [0 pi 2*pi 0];
x_dp_g      = state_calc(x0,parms);
...
function [x_dp] = state_calc(x0,parms)
