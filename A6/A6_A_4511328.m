...
[t23,q23] = ode23(@(t,q) ODE_func(t,q), [0 time], q0',opt);
...
tic
opt = odeset('AbsTol',1e-6,'RelTol',1e-6,'Stats','on');
[t45,q45] = ode45(@(t,q) ODE_func(t,q), [0 time], q0',opt);
...
tic
opt = odeset('AbsTol',1e-6,'RelTol',1e-6,'Stats','on');
[t113,q113] = ode113(@(t,q) ODE_func(t,q), [0 time], q0',opt);
t113_mean    = mean(diff(t113));                                            % Caculate mean step size
disp(t113_mean);
toc
...
% state.
function [t,q] = ODE_custom(time,q0,method,parms)
...
switch method
    ...
    case 'euler'
        ...
        for ii = 1:(size(t,1)-1)
            q_now_tmp       = num2cell(q(ii,1:end-2),1);                  % Create cell for subs function function
            qdp             = subs_qdp(q_now_tmp{:}).';                   % Calculate the second derivative of the generalised coordinates
            q(ii,end-1:end) = qdp;
            q(ii+1,1:end-2) = q(ii,1:end-2) + parms.h*q(ii,3:end);        % Perform euler intergration step
            
            % Calculate last acceleration
            if ii == (size(t,1)-1)
                q_next            = num2cell(q(ii+1,1:end-2),1);          % Create cell for subs function function
                q(ii+1,end-1:end) = subs_qdp(q_next{:}).';                % Calculate the second derivative of the last step
            end
        end
    ...
    case 'heun'
        ...
        for ii = 1:(size(t,1)-1)
            ...
            q_now_tmp       = num2cell(q_now,1);                                % Create cell for subs function function
            qdp_now_tmp     = subs_qdp(q_now_tmp{1:end-2}).';                   % Calculate the second derivative of the generalised coordinates
            ...
            q_star_tmp      = num2cell(q_star,1);                               % Create cell for subs function function
            qdp_star_tmp    = subs_qdp(q_star_tmp{:}).';                        % Calculate the second derivative of the generalised coordinates
            ...
            % Calculate last acceleration
            if ii == (size(t,1)-1)
                q_next_tmp        = num2cell(q(ii+1,1:end-2),1);                % Create cell for subs function function
                q(ii+1,end-1:end) = subs_qdp(q_next_tmp{:}).';                  % Calculate the second derivative of the last step
            end
        end
    ...
    case 'RK3'
        for ii = 1:(size(t,1)-1)
            q_now_tmp         = num2cell(q(ii,1:end-2),1);                                               % Create cell for subs function function
            K1                = [cell2mat(q_now_tmp(1,end-1:end)),subs_qdp(q_now_tmp{:}).'];             % Calculate the second derivative at the start of the step
            q1_tmp            = num2cell(cell2mat(q_now_tmp) + (parms.h*0.5)*K1);                        % Create cell for subs function function
            K2                = [cell2mat(q1_tmp(1,end-1:end)),subs_qdp(q1_tmp{:}).'];                   % Calculate the second derivative halfway the step
            q2_tmp            = num2cell(cell2mat(q_now_tmp) + ((parms.h*0.75))*K2);                     % Refine value calculation with new found derivative
            K3                = [cell2mat(q2_tmp(1,end-1:end)),subs_qdp(q2_tmp{:}).'];                   % Calculate new derivative at the new refined location
            ...
            % Calculate last acceleration
            if ii == (size(t,1)-1)
                q_now_tmp         = num2cell(q(ii+1,1:end-2),1);                                             % Create cell for subs function function
                K1                = [cell2mat(q_now_tmp(1,end-1:end)),subs_qdp(q_now_tmp{:}).'];             % Calculate the second derivative at the start of the step
                q1_tmp            = num2cell(cell2mat(q_now_tmp) + (parms.h*0.5)*K1);                        % Create cell for subs function function
                K2                = [cell2mat(q1_tmp(1,end-1:end)),subs_qdp(q1_tmp{:}).'];                   % Calculate the second derivative halfway the step
                q2_tmp            = num2cell(cell2mat(q_now_tmp) + ((parms.h*0.75))*K2);                     % Refine value calculation with new found derivative
    ...
    case 'RK4'
        for ii = 1:(size(t,1)-1)
            ...
            K4                = [cell2mat(q3_tmp(1,end-1:end)),subs_qdp(q3_tmp{:}).'];                   % Calculate last second derivative
...
% xdp_handle       = matlabFunction(xdp,'File','subs_xdp');                         % Create function handle of EOM in terms of COM positions
matlabFunction(simplify(qdp),'File','subs_qdp');                                    % Create function handle of EOM in terms of generalised coordinates
end